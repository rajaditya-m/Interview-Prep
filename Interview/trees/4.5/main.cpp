...
bool isBinarySearchTree(TreeNode* root, int minRange, int maxRange)
{
	if(!root)
		return true;
	else if((root->value() < maxRange) && (root->value() >= minRange))
	{
		return isBinarySearchTree(root->getLeftChild(),minRange,root->value()) && 
		isBinarySearchTree(root->getRightChild(),(root->value()),maxRange);
	}
	else
		return false;
}
